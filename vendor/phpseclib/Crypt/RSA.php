...
namespace MeowCaster_Vendor\phpseclib\Crypt;
...
class RSA {
	...
	/**
	 * Use
	 * {@link http://en.wikipedia.org/wiki/Optimal_Asymmetric_Encryption_Padding Optimal Asymmetric Encryption Padding}
	 ...
	 * Use PKCS#1 padding.
	 *
	 * Although self::ENCRYPTION_OAEP offers more security, including PKCS#1 padding is necessary for purposes of
	 * backwards compatibility with protocols (like SSH-1) written before OAEP's introduction.
	 */
	const ENCRYPTION_PKCS1 = 2;
	/**
	 * Do not use any padding
	 *
	 * Although this method is not recommended it can none-the-less sometimes be useful if you're trying to decrypt
	 * some legacy stuff, if you're trying to diagnose why an encrypted message isn't decrypting, etc.
	 */
	 ...
	 * \phpseclib\Crypt\RSA doesn't do it is because OpenSSL doesn't fail gracefully.  openssl_pkey_new(), in
	 * particular, requires openssl.cnf be present somewhere and, unfortunately, the only real way to find out is too
	 * late.
	 *
	 ...
	 *                  Will need to be passed back to \phpseclib\Crypt\RSA::createKey() as the third parameter for
	 *                  further processing.
	 *
	 * @access public
	 *
	 ...
	 * include the public exponent this won't work unless you manually add the public exponent. phpseclib tries to
	 * guess if the key being used is the public key but in the event that it guesses incorrectly you might still want
	 ...
	 * @access public
	 *
	 * @param string $algorithm The hashing algorithm to be used. Valid options are 'md5' and 'sha256'. False is
	 ...
	 *
	 * Used with signature production / verification and (if the encryption mode is self::ENCRYPTION_OAEP) encryption
	 * and decryption.  If $hash isn't supported, sha1 is used.
	 *
	 * @access public
	 *
	 * @param string $hash
	 ...
	 *
	 * See
	 * {@link http://codahale.com/a-lesson-in-timing-attacks/ A Lesson In Timing Attacks (or, Don't use MessageDigest.isEquals)}
	 *
