...
use MeowCaster_Vendor\phpseclib\Crypt\Base;
use MeowCaster_Vendor\phpseclib\Crypt\Blowfish;
use MeowCaster_Vendor\phpseclib\Crypt\Hash;
use MeowCaster_Vendor\phpseclib\Crypt\Random;
use MeowCaster_Vendor\phpseclib\Crypt\RC4;
use MeowCaster_Vendor\phpseclib\Crypt\Rijndael;
use MeowCaster_Vendor\phpseclib\Crypt\RSA;
use MeowCaster_Vendor\phpseclib\Crypt\TripleDES;
use MeowCaster_Vendor\phpseclib\Crypt\Twofish;
use MeowCaster_Vendor\phpseclib\Math\BigInteger;
use MeowCaster_Vendor\phpseclib\System\SSH\Agent;
...
class SSH2 {
	 ...
	 *
	 * We need to know how big the HMAC will be for the server to client direction so that we know how many bytes to
	 * read. For the client to server side, the HMAC object will make the HMAC as long as it needs to be.  All we need
	 * to do is append it.
	 *
	 * @see    self::_get_binary_packet()
	 * @var int
	 ...
	 *           {@link http://tools.ietf.org/html/rfc4251#section-9.3.9 traffic analysis} by sending dummy
	 *           SSH_MSG_IGNORE messages.
	 */
	 ...
	 *
	 * See {@link http://tools.ietf.org/html/rfc4256 RFC4256} for details.  This is not a full-featured
	 ...
	 * Set Timeout
	 *
	 * $ssh->exec('ping 127.0.0.1'); on a Linux host will never return and will run indefinitely.  setTimeout() makes
	 * it so it'll timeout. Setting $timeout to false or 0 will mean there is no timeout.
	 *
	 * @param mixed $timeout
	 *
	 * @access public
	 ...
	 * Execute Command
	 *
	 * If $callback is set to false then \phpseclib\Net\SSH2::_get_channel_packet(self::CHANNEL_EXEC) will need to be
	 * called manually. In all likelihood, this is not a feature you want to be taking advantage of.
	 *
	 ...
	 * Start a subsystem.
	 *
	 * Right now only one subsystem at a time is supported. To support multiple subsystem's stopSubsystem() could
	 * accept
	 * a string that contained the name of the subsystem, but at that point, only one subsystem of each type could be
	 * opened. To support multiple subsystem's of the same name maybe it'd be best if startSubsystem() generated a new
	 * channel id and returns that and then that that was passed into stopSubsystem() but that'll be saved for a future
	 * date and implemented if there's sufficient demand for such a feature.
	 *
	 ...
	 *
	 * Returns a string if NET_SSH2_LOGGING == self::LOG_COMPLEX, an array if NET_SSH2_LOGGING == self::LOG_SIMPLE and
	 * false if !defined('NET_SSH2_LOGGING')
	 *
	 * @access public
	...
	/**
	 * Return a list of the (symmetric key) encryption algorithms the server supports, when receiving stuff from the
	 * client.
	 *
	 * @return array
	 * @access public
