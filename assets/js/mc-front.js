...
var meowPrefixClass = 'mcss-',
    ...
    meowEmbedClass = meowPrefixClass + 'embed-';

function meowRand(max) {
    return Math.floor(Math.random() * Math.floor(max));
}

function meowValidateConsent(type_consent, duration) {

    var expires = "",
        cookie_name = "_mc_save_consent"; // in days

    var date = new Date();
    date.setTime(date.getTime() + duration * 24 * 60 * 60 * 1000);
    expires = "; expires=" + date.toUTCString();

    document.cookie = cookie_name + "=" + type_consent + expires + "; path=/";
}

function meowCheckConsent() {
    var cookie_name = "_mc_save_consent",
        nameEQ = cookie_name + "=",
...
function meowVideosConsent(videos_consent) {

    document.querySelectorAll('button', videos_consent).forEach(function (a) {

        a.addEventListener('click', function (evt) {

            meowValidateConsent(this.value, this.getAttribute('data-duration'));
            window.location.reload(false);
        });
    });
}

function meowToFrame(el, lazy) {
    if (!lazy) {
        var lazy = false;
    }
    var el = el;
    // Create an iFrame with the url predefine with the param too
    var iframe = document.createElement("iframe");
    var iframeID = meowEmbedClass + "youtube-" + el.getAttribute('data-id') + '-' + meowRand(100);
    iframe.setAttribute("src", el.getAttribute('data-url'));
    iframe.setAttribute("frameborder", '0');
    iframe.setAttribute("id", iframeID);
    // The height and width of the iFrame should be the same as parent
    iframe.setAttribute('width', el.offsetWidth);
    if (el.offsetHeight === 0) {
        iframe.setAttribute('height', el.offsetWidth / 16 * 9);
    } else {
        iframe.setAttribute('height', el.offsetHeight);
    }
    if (lazy) {
...
function meowVideos(videos) {
    var nb_videos = videos.length;
    for (var i = 0; i < nb_videos; i++) {
        // Play button first element for stay visible without play with z-index
        ...
        imgBase.setAttribute("class", meowPrefixClass + 'youtube-img ' + meowPrefixClass + 'youtube-img-' + videos[i].getAttribute('data-id'));
        // 4:3
        //imgBase.setAttribute("src", 'http://i.ytimg.com/vi/' + videos[i].getAttribute('data-id') + '/sddefault.jpg' );
        // 16:9 small
        imgBase.setAttribute("src", 'https://i.ytimg.com/vi/' + videos[i].getAttribute('data-id') + '/mqdefault.jpg');
        // 16:9 HD
        //imgBase.setAttribute("src", 'https://i.ytimg.com/vi/' + videos[i].getAttribute('data-id') + '/maxresdefault.jpg' );


        videos[i].appendChild(imgBase);
        objectFitImages(imgBase);
        if (videos[i].getAttribute('data-widget') === "1") {
            noframe(videos[i]);
        }
        //reframe(meowPrefixClassDot + 'youtube-img-' + videos[i].getAttribute('data-id'));
        // Overlay the Play icon to make it look like a video player

        videos[i].onclick = function () {
            meowToFrame(this, true);
        };
    }
}
...
r(function () {
    ...
    // For gallery
    if (document.querySelectorAll('.mcss-vgc').length > 0) {
        var meow_glightbox = GLightbox({
            'selector': 'glightbox'
        });
    }

    var videos_consent = document.querySelectorAll(".mcss-player-consent-container");

    if (videos_consent.length > 0) {
        var meowConsent = meowCheckConsent();
        if (meowConsent === null) {
            // AddEventListener
            meowVideosConsent(videos_consent);
        } else {
            console.log('error');
        }
    } else {

        var videos = document.getElementsByClassName(meowEmbedClass + "youtube");
        if (videos) {
            meowVideos(videos);
        }
    }
